...
/**
 * Class EditTariffLogic ...
 *
 * @author Виталий
 * Created on 21.06.2016
 */
public class EditTariffLogic {
    /** Field PARAM_NAME_VALIDATION  */
    private static final String PARAM_TEXT_VALIDATION = "^[A-ZА-ЯЁ][A-ZА-ЯЁa-zа-яё+-_\\s]{2,}$";
     ...
     */
    public static void editTariff(String requestData, String locale) throws TechnicalException, LogicException {
        Tariff tariff = null;
        try {
            JSONObject json = stringToJson(requestData);
            tariff = jsonToTariff(json);
            tariff.setId(jsonToId(json));
            validation(tariff, locale);
            TariffDAO.getInstance().editTariff(tariff);
